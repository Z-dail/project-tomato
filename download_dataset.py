...
def get_dataframe(filelist=None, labels_dict=None):
    ...
    if filelist is None or len(filelist) == 0:
        ...
        for filepath in filelist:
            filenames.append(filepath)
            # 使用 basename 和 dirname 避免路径索引问题
            dir_name = os.path.basename(os.path.dirname(filepath))
            label = dir_name.split('___')[1] if '___' in dir_name else "Unknown"
...
#         except Exception as exp:
#             print(exp)
def scanOldDataset(dataDir="Plant_leave_diseases_dataset_without_augmentation"):
    # 直接扫描解压后的原始目录
    all_folders = glob.glob(os.path.join(dataDir, "Tomato_*", "*"))

    # 统计文件数
    print(f"Total Tomato Files: {len(all_folders)}")

    # 这里需要实现数据集划分逻辑（训练集、验证集、测试集）
    # 当前代码缺少实际的划分逻辑，只是简单列出所有文件

    # 划分数据集（示例：按80-10-10比例）
    import random
    random.shuffle(all_folders)

    train_split = int(0.8 * len(all_folders))
    val_split = int(0.9 * len(all_folders))

    train_files = all_folders[:train_split]
    val_files = all_folders[train_split:val_split]
    test_files = all_folders[val_split:]
    ...
    def get_relative_paths(file_list, base_dir):
        return [os.path.relpath(f, base_dir) for f in file_list]

    train_relative = get_relative_paths(train_files, dataDir)
    val_relative = get_relative_paths(val_files, dataDir)
    test_relative = get_relative_paths(test_files, dataDir)

    for split_name, files in [("train", train_relative), ("valid", val_relative), ("test", test_relative)]:
        with open(os.path.join("splits", f"{split_name}.txt"), "w") as f:
            f.write("\n".join(files))
...
if __name__ == "__main__":
    ...
    # datasetUrl = "E:\python\project-tomato"
